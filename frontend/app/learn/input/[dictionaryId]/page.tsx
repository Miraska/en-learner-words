...
export default function LearnInputMode({
    params,
    searchParams,
}: {
    ...
    const { filters, updateFilters } = useUrlFilters({
        storageKey: `learn-input-filters-${dictionaryId}`,
        defaultFilters: {
            exclude: false,
            hide: 'target',
            limit: null,
  ...
  const saveSession = useMutation({
    ...
    const storageKey = `wordcraft-game-${dictionaryId}-input-${hideMode}`;
    ...
    // (логика перезагрузки обрабатывается в блоке Multiplayer wiring)
    // Ранее здесь проверялось сохранённое состояние и показывалось предупреждение.
    ...
    const filteredWords: Word[] = useMemo(() => {
        ...
        // Не скрываем слова по языку — hideMode влияет только на то, что показываем/что вводим
        ...
        return filtered;
    }, [words, learnedEntries, excludeLearned]);
    ...
    useEffect(() => {
        ...
        joinRoom({ roomId, userId, email, dictionaryId, mode: 'input', wordOrder });
        ...
        onState((s) => {
            ...
            // Проверяем, готовы ли все игроки для автоматического запуска
        ...
        onStarted(({ wordOrder, startedAt }) => {
            ...
            // Скрываем возможное предупреждение о перезагрузке при старте игры
    ...
    useEffect(() => {
        if (!hasMounted) return;
        if (roomId) return;
        if (finished) return; // don't auto-restart after finish
        if (!Array.isArray(shuffledWords) || shuffledWords.length === 0) return;
        if (!gameStarted) {
    ...
    }, [hasMounted, roomId, shuffledWords, gameStarted, finished]);

    // Сохранение состояния при изменениях
    useEffect(() => {
        if (!words || !words.length) return;
        saveGameState();
    }, [currentIndex, correctCount, resultsByIndex, repeatResultsByIndex, finished, repeatWrong, words]);
    ...
    const wordsToStudy: Word[] = useMemo(() => {
        ...
        // Get words that были ошибочными в первой сессии
    ...
    const checkAnswer = () => {
        ...
        // Ожидаемая сторона: при hide=source ожидаем source слово, иначе ожидаем перевод(ы)
  ...
  const handleSubmit = async () => {
        ...
        if (revealed) {
            ...
            if (isLast) {
                ...
                if (!roomId) {
                  try {
                    // When revealed path, the last answer has already been set in lastAnsweredCorrect
                    const recalledFinal = correctCount + (lastAnsweredCorrect ? 1 : 0);
                    await saveSession.mutateAsync({
                      dictionaryId,
                      recalled: recalledFinal,
                      notRecalled: (total) - recalledFinal,
                      unknown: 0,
                      mode: 'input',
                      isMultiplayer: false,
                    });
        ...
        // Report progress to multiplayer
        if (roomId) {
    ...
    setTimeout(async () => {
            ...
            if (!roomId) {
              try {
                await saveSession.mutateAsync({
                  dictionaryId,
                  recalled: correctCount + (lastAnsweredCorrect ? 1 : 0),
                  notRecalled: (total) - (correctCount + (lastAnsweredCorrect ? 1 : 0)),
                  unknown: 0,
                  mode: 'input',
                  isMultiplayer: false,
                });
    ...
    if (finished) {
        let totalCount, correctCountFinal, wrongCount, percent;
        if (repeatWrong) {
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 justify-center">
                                <button
                                    ...
                                    onClick={() => {
                                        // restart single-player input session
                                        setFinished(false);
                                        setCurrentIndex(0);
                                        setResultsByIndex([]);
                                        setRepeatResultsByIndex([]);
    ...
    return (
        <div className="flex flex-col min-[660px]:flex-row justify-center items-center min-h-screen p-4 gap-4">
